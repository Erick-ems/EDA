...
void redimensionaFila(fila *f){
  ...
  for(i = 0, j = f->p; j != f->u; i++, j = (j + 1) % f->N){ // o loop for vai percorrer a lista original e copiar para a nova lista, e ao chegar ao final reiniciar a contagem, por se tratar de uma lista circular
...
int desenfileira(fila *f, int *y){
  ...
  *y = f->dados[f->p]; // pegamos o endereço do dado da primeira posição e passamos como refencia para o y
  ...
  f->p = (f->p + 1) % f->N; // atualizamos o primeiro elemento para o proximo
